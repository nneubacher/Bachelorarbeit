...
def setup_collection(client, embedding_function):
    return client.get_or_create_collection(name="data", metadata={"hnsw:space": "cosine"}, embedding_function=embedding_function)
...
def main():
    ...
    squad_loader = SQuADLoaderV1Parquet(os.path.join(os.path.dirname(os.path.abspath("__file__")), 'data/squad_test.parquet'))
